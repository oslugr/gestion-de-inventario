...
function eliminarUsuario(username, posicion){
	$.ajax({
		...
		success: function(){
			...
			if(usuarios.cantidad%10 == 0 && pagina!=1){
				pagina--;
			}

			usuarios.data.splice(posicion,1);
			crearUsuarios();
			generarPie();

			// Actualiza la tarjeta superior
			$('#elementos-totales-tarjeta').html(usuarios.data.length);
		},
		error: function(){
			if(username != 'admin')
				$('#titulo-error').html('Error al eliminar el usuarios')
			else
				$('#titulo-error').html('No se puede eliminar el admin')

			$('#mensaje-error').html('Ha ocurrido un error al eliminar el usuario')
