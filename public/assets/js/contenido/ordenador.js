...
if(urlParams.has('id')){
    $.get(`/api/ordenador/info/${urlParams.get('id')}`, crearInterfazOrdenador);
}
else{
    window.location.replace('../pages/404.html')
}

function crearInterfazOrdenador(data){
    ordenador = data;
...
function mostrarOrdenador(){
    ...
    if (ordenador.observaciones == null)        ordenador.observaciones = "";
	if (ordenador.localizacion_taller  == null) ordenador.localizacion_taller = "";

    $("#main-contenido").append(`
		<div id="tabla-ordenador" class="w-full overflow-hidden rounded-lg shadow-xs mt-10">
			...
			<div class="w-full overflow-x-auto">
				<table class="w-full whitespace-no-wrap">
					...
					<tbody id="body-tabla-ordenador" class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
					
                    <tr id="ordenador" class=" cursor-pointer ordenador text-gray-700 dark:text-gray-400" onclick="cargarInfoOrdenador(id, this)">
                        <td class="px-4 py-3">
                            <div class="flex items-center text-sm">
                                ...
                                <div>
                                    <p class="font-semibold">${ordenador.id}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3 text-sm">
                            ${ordenador.localizacion_taller}
                        </td>
                        <td class="px-4 py-3 text-sm">
                            ${ordenador.observaciones}
                        </td>
                    </tr>
...
function aniadirCaracteristicaNueva(num){
	$('#body-tabla-caracteristicas').append(`
		<tr id="fila-caracteristica-nueva-${num}" class="fila-caracteristica-nueva text-gray-700 dark:text-gray-400">
			<td class="px-2 py-3 text-sm">
				<input id="caracteristica-nueva-${num}-nombre" class="w-32 dark:border-gray-600 dark:bg-gray-800 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray" type="text" value=""/>
			</td>
			<td class="px-2 py-3 text-sm">
				<input id="caracteristica-nueva-${num}-valor" class="w-32 dark:border-gray-600 dark:bg-gray-800 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray" type="text" value=""/>
			</td>
			<td class="px-2 py-3">
			
			<div class="flex items-center space-x-4 text-sm">
				<button @click="openModal" id="icono-borrar-caracteristica-nueva-${num}" onclick="eliminarCaracteristicaNueva(${num});" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray">
					<svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
						<path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"></path>
					</svg>
				</button>
	...
	$('#boton-nueva-caracteristica').attr('onclick', `aniadirCaracteristicaNueva(${num+1})`);

}

function eliminarCaracteristicaNueva(num){
	$(`#fila-caracteristica-nueva-${num}`).remove();
}
