$.get("/api/recogida/Recogida", crearInterfazRecogidas);
...
function fila(id, fecha, localizacion, posicion) {
	...
	return `
		<tr id="recogida-${id}" class="recogida text-gray-700 dark:text-gray-400">
			...
			<td id="fecha-${id}" class="px-4 py-3 text-xs">
                ${fecha.split('T')[0]}
			</td>
			<td id="localizacion-${id}" class="px-4 py-3 text-xs">
				${localizacion}
			</td>
			<td class="px-4 py-3">
...
function eliminarRecogida(id, posicion){
	$.ajax({
		...
		error: function(){
			...
			$('#mensaje-error').html('Se ha producido un error al eliminar la recogida')
...
function editarRecogida(id, posicion){
	...
	$.ajax({
		...
		error: function(){
			...
			$('#mensaje-error').html('Se ha producido un error al editar la recogida')
			$('.popup').removeClass('hidden');
